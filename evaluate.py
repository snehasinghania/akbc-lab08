    ...
    for fname in input_dir.glob("*.csv"):
        ...
        for entity, ground_truth_objects in ground_truth_df.groupby(["SubjectEntity"])[
            ...
            if len(predictions) == 0:
                ...
                res_df.append(
                    {"SubjectEntity": entity, "Relation": relation, "Precision": 0, "Recall": 0, "F1-score": 0}
                )
            else:
                ...
                res_df.append(
                    {"SubjectEntity": entity, "Relation": relation, "Precision": p, "Recall": r, "F1-score": f1}
                )
        ...
        average_p[relation] = res_df["Precision"].mean()
        average_r[relation] = res_df["Recall"].mean()
        average_f1[relation] = res_df["F1-score"].mean()

    ### calculating the final F1-score averaged across all the relations
    ### NOTE: this score will be used to rank the submissions
    p = round(np.mean(list(average_p.values())) * 100, 2)
    r = round(np.mean(list(average_r.values())) * 100, 2)
    f1 = round(np.mean(list(average_f1.values())) * 100, 2)

    print("average Precision, Recall, and F1-score for each relation:")
    pprint.pprint(average_p)
    pprint.pprint(average_r)
    pprint.pprint(average_f1)

    print("Final macro-precision: {} %".format(p))
    print("Final macro-recall: {} %".format(r))
    print("Final macro-F1-score: {} %".format(f1))
